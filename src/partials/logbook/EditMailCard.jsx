...
export default function EditMailCard({ mailId, type, onClose, onUpdated }) {
  ...
  if (loading) return <div className="p-6 text-center">Memuat data surat...</div>;
  ...
  return (
    <div className="col-span-full bg-white dark:bg-gray-800 rounded-xl shadow">
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Shared Fields */}
        <Input name="number" label="Nomor Surat" value={form.number} onChange={handleChange} />
        <Select name="category" label="Kategori" value={form.category} onChange={handleChange} options={categories} />

        <Input type="date" name="date" label="Tanggal Surat" value={form.date} onChange={handleChange} />

        {type === 'inbox' ? (
          <>
            <Input type="date" name="recievedDate" label="Tanggal Diterima" value={form.recievedDate} onChange={handleChange} />
            <Input name="origin" label="Asal Surat" value={form.origin} onChange={handleChange} />
            <TextArea name="summary" label="Ringkasan Isi" value={form.summary} onChange={handleChange} />
          </>
        ) : (
          <>
            <Input name="destination" label="Tujuan Surat" value={form.destination} onChange={handleChange} />
            <TextArea name="summary" label="Ringkasan Isi" value={form.summary} onChange={handleChange} />
            ...
            <Input name="sign" label="Penandatangan" value={form.sign} onChange={handleChange} />
          </>
        )}

        <div className="md:col-span-2">
          <label className="block text-sm text-gray-700 dark:text-gray-300 mb-2">Lampiran Baru (opsional)</label>
          <input
            type="file"
            name="attachments"
            multiple
